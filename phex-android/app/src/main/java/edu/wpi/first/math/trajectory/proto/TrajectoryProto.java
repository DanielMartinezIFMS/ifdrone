...
public class TrajectoryProto implements Protobuf<Trajectory, ProtobufTrajectory> {
  ...
  public void pack(ProtobufTrajectory msg, Trajectory value) {
    us.hebi.quickbuf.RepeatedMessage<ProtobufTrajectoryState> states = msg.getMutableStates().reserve(value.getStates().size());
    for (Trajectory.State item : value.getStates()) {
